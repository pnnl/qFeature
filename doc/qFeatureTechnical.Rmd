...
#qFeature
The goal of this guide is to be used as a quick start guide for using `qFeature`package. This guide begins with a quick installation guide in 'Getting Started'. Following installation is a very basic diagram of the 'Package Structure' to illustrate the heirarchy of functions. Finally, this document finishes off by providing details on using the 4 'Core Functions' contained within the package complete with follow-along examples.

##Description
The `qFeature` package written by Landon Sego is designed to produce a multivariate signature from continuous and discrete variables contained within a time series dataset. This is a high-speed implementation of the feature extraction methods of the Morning Report Algorithms developed by Brett Amidan and Tom Ferryman.
...
* Coefficient estimate for the intercept equals to the second value of __a__
* Coefficient estimate for x1 equals to the second value of __b__
* Coefficient estimate of I(x1^2) equals to the second value of __c__
* Residual standard error equals to the second value of __d__
...
####Description
The `getFeatures()` function is the main workhorse of the `qFeature()` package. It combines the capability of `fitQ()` and `discFeatures()` in order to fit a moving window quadratic (or linear) regression model for each continuous variable in a data frame, and calculates summary statistics of the parameters. Additionally it calculates the duration and transition features of discrete variables.


####How to use getFeatures()
At this point it is important that you understand what is happening in both the `fitQ()` and the `discFeatures()` functions, because `getFeatures()` is simply an aggregate of the two. Rather than dealing with a single vector at a time, this function is capable of dealing with a dataframe consisting of multiple continuous and/or discrete variables. The only additional functionality introduced by the `getFeatures()` function is the ability to output desired summary statistics of the features extracted from the regression models.
...
* __[stat]__ identifies the summary statistic being applied to all values of the specified feature.
 ...
 In our example we include mean and standard deviation for all the values extracted for that feature. There are many more summary statistics that could be included that are listed in the `getFeatures()` [help page](http://martingale.pnl.gov/computing/repos/html_help/qFeature/html/getFeatures.html "getFeatures() - R Help Page").
...
* __[variable]__ Identifies the discrete variable of interest from the data
    * disc1
    * disc2
* __[frequency]__ Either the percent of time at a given state or a count of the transitions from one state to another.
    * percent
    * num_trans
...
####Description
The `ddply_getFeatures()` function is simply a wrapper that allows the `getFeatures()` function to be implemented for each "group" or unique value for a specified categorical variable or combination of categorical variables in a dataframe using the `ddply()` function from the `plyr` package. 
...
Now that we have a data set let's pretend we are interested in every combination of subject and phase. At this point it is important to note that in order to work properly, the package expects the data it is working with to be structured in several ways.  First,it is essential that any data set being processed by this package contain a field which indicating to which phase each record belongs. Second, the package assumes that the user's data is presented in chronological order (from oldest to newest) throughout each phase.  Once you've ensured that your data is structured, you can then proceed to process it through the `ddply_getFeatures` function. The output will be very similar to `getFeatures` however in this case instead of getting one value for each summary, we will end up with a set of values for each summary equal to the number of unique combinations. A quick calculation shows us that:
...
Hopefully now you have the knowledge required to implement any of the core functions sucessfully in your own data set.  As you use the package, bear in mind that the end output of `qFeature` does not need to be the final analytical product in all applications, but may also be used as an exploratory analytical product that can be used in subsequent analysis to either better understand the structure of the data itself, or to identify anomalous records. This can entail almost anything from Principal Component Analysis, to plotting the output of `qFeature` in a standard line or scatter plot, or any other visualization the user prefers.    