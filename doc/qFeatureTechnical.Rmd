...
#qFeature
The goal of this guide is to be used as a quick start guide for using `qFeature`package. This guide 
begins with a quick installation guide in 'Getting Started'. Following installation is a very basic 
diagram of the 'Package Structure' to illustrate the heirarchy of functions. Finally, this document 
finishes off by providing details on using the 4 'Core Functions' contained within the package complete with follow-along examples.

##Description
The `qFeature` package written by Landon Sego is designed to produce a multivariate signature from 
continuous and discrete variables contained within a time series dataset. This is a high-speed 
implementation of the feature extraction methods of the Morning Report Algorithms developed by Brett 
Amidan and Tom Ferryman.

##Getting Started

This guide is structured with the assumption that you are beginning with a basic understanding of R, 
and aims to provide an understandable blueprint for implementing the tools contained within the 
`qFeature` package with your own data.

The first thing you will need to do is install the `qFeature` package. You will need access to the 
[martingale repository](http://martingale.pnl.gov/computing/repos/) in order to obtain the package. 
After permissions for access have been granted, the package can then be installed by:

```{r eval=FALSE}
install.packages("qFeature", repos="http://martingale.pnl.gov/computing/repos")
```

Once the package has been installed, the library can be loaded so that the functions can be used. A 
complete list of the functions contained within the `calcIVT` package can be seen using:
...
The data moves through the window by an increment of `skip` which has a default of 1 and fits a model 
...
Futhermore, the first values of **a**, **b**, **c**, and **d** are all drawn from the same window as 
...
####Description
The `getFeatures()` function is the main workhorse of the `qFeature()` package. It combines the 
capability of `fitQ()` and `discFeatures()` in order to fit a moving window quadratic (or linear) 
regression model for each continuous variable in a data frame, and calculates summary statistics of 
the parameters. Additionally it calculates the duration and transition features of discrete variables.
...
dealing with a single vector at a time, this function is capable of dealing with a dataframe 
...
[help page](http://martingale.pnl.gov/computing/repos/html_help/qFeature/html/getFeatures.html 
"getFeatures() - R Help Page").
...
implemented for each "group" or unique value for a specified categorical variable or combination of 
categorical variables in a dataframe using the `ddply()` function from the `plyr` package. 
...
associated. Second, the package assumes that the user's data is presented in chronological order (from 
...
Hopefully now you have the knowledge required to implement any of the core functions sucessfully in 
your own data set.  As you use the package, bear in mind that the end output of `qFeature` does not 
need to be the final analytical product in all applications, but may also be used as an exploratory 
analytical product that can be used in subsequent analysis to either better understand the structure 
of the data itself, or to identify anomalous records. This can entail almost anything from Principal 
Component Analysis, to plotting the output of `qFeature` in a standard line or scatter plot, or any 
other visualization the user prefers.
