...
1. Cite Amidan's paper in the first Description
2. Change all martingale help links to point at the appropriate github links, which are still
   to be determined.
3. Verify 1st person is used throughout
4. Spell check this Rmd file
-->

#qFeature

Lucas Tate, Landon Sego, Ryan Butner
...
diagram of the 'Package Structure' to illustrate the heirarchy of functions. Finally, we 
...
Amidan and Tom Ferryman. [Cite Amidan Paper here]

##Getting Started

The first thing you will need to do is install the `qFeature` package. 

```{r eval=FALSE}
install.packages("devtools") # if not installed
library(devtools)
install_github("pnnl/qFeature")
```

Once the package has been installed, the library can be loaded so that the functions can be used. A 
complete list of the functions contained within the `qFeature` package can be seen using:
...
against a data frame, where `fitQ()` is applied to the continous variables and `discFeatures()` is
